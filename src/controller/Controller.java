...
public class Controller {
    ...
    private static final String accountRegExp = "^[0-9,a-z,A-Z,\\u4e00-\\u9fa5]{1,15}$";
    private static final String nameRegExp = "^[a-z,A-Z,\\u4e00-\\u9fa5]{1,100}$";
    private static final String phoneRegExp = "^(((13[0-9])|(15[0-3][5-9])|(18[0,5-9]))\\d{8})|(0\\d{2}-\\d{8})|(0\\d{3}-\\d{7})$";
    private static final String passwordRegExp = "^[a-z,A-Z,0-9]{6,20}$";
    ...
    public void exec() {
        ...
        headPortrait.setModality(alterPerson);
     ...
     * @return
     */
    private Object $(WindowStage WindowStage, String id) {
        return WindowStage.getRoot().lookup("#" + id);
    }
                    ...
                    if (resultSet.next()) {
                        if (resultSet.getString(3).equals(Password)) {
                            ...
                            if (set.next()) {
                                ...
                                //聊天助手
                                mainWindow.addFriend("system", "Gossip聊天助手", "聊天助手");
                                ((Label) $(mainWindow, "Y_account")).setText("Gossip聊天助手");
                                MsgData.msg.add(new Vector<>());
                                MsgData.accountList.add("Gossip聊天助手");
                                MsgData.msgTip.put("Gossip聊天助手", 0);
                                //所有好友
                                ...
                                mainWindow.addLeft("system", "欢迎使用Gossip,赶快找好友聊天吧!");
                                MsgData.msg.get(0).add("Gossip聊天助手 欢迎使用Gossip,赶快找好友聊天吧!");
                                //输入框禁用
    ...
    private void sendMsgExec() {
        ((Button) $(mainWindow, "send")).setOnAction(event -> {
            ...
            try {
                ...
                if (resultSet.next()) {
                    String input = ((TextArea) $(mainWindow, "input")).getText();
                    if (!input.equals("")) {
                        //加密内容
                        String line = userdata.getAccount() + " " + youAccount + " " + Cypher.encode(input);
                        //添加自己的消息
                        ...
                        try {
                            ...
                            //清输入框
                            ((TextArea) $(mainWindow, "input")).clear();
                        } catch (IOException e) {
        ...
        ((TextArea) $(mainWindow, "input")).setOnKeyPressed(event -> {
            if (event.isShiftDown() && event.getCode() == KeyCode.ENTER) {
                //System.out.println(((TextArea) $(mainWindow, "input")).getText());
                String youAccount = MsgData.accountList.get(mainWindow.getFriendList().getSelectionModel().getSelectedIndex());
                try {
                    //选择登入的好友
                    ResultSet resultSet = database.execResult("SELECT * FROM dialog WHERE account=?", youAccount);
                    if (resultSet.next()) {
                        ...
                        if (!input.equals("")) {
                            ...
                            try {
                                ...
                                //清输入框
                                ((TextArea) $(mainWindow, "input")).setText(null);
                                ((TextArea) $(mainWindow, "input")).clear();
                            } catch (IOException e) {
                                alert.setInformation("你断开了链接!");
                                alert.exec();
    ...
    private void forgetExec() {
        ...
        ((Button) $(forget, "reset")).setOnAction(event -> {
            ...
            if (account.equals("") || name.equals("") || phone.equals("") || password.equals("") || rePassword.equals("")) {
            ...
            } else if (!Pattern.matches(accountRegExp, account) || !Pattern.matches(nameRegExp, name) || !Pattern.matches(phoneRegExp, phone) || !Pattern.matches(passwordRegExp, password) || !Pattern.matches(passwordRegExp, rePassword)) {
                ...
                if (!Pattern.matches(passwordRegExp, rePassword)) {
    ...
    public void registerExec() {
        ((Button) $(register, "register")).setOnAction(event -> {
            ...
            } else if (!Pattern.matches(accountRegExp, account) || !Pattern.matches(nameRegExp, name) || !Pattern.matches(passwordRegExp, password) || !Pattern.matches(passwordRegExp, rePassword) || !Pattern.matches(ageRegExp, age) || !Pattern.matches(phoneRegExp, phone)) {
                if (!Pattern.matches(accountRegExp, account)) {
    ...
    public static void setHeadPortrait(Button button, String file, String bg) {
        button.setStyle(String.format("-fx-background-image: url('/view/css/image/%s/%s.jpg')", file, bg));
    }
